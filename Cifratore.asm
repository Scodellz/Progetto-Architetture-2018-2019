...
prossimoAlg: 	lb 	$t0, ($s6)		# Carico l'elemento puntato della chiave
		...
		sub	$t0, $t0, $t1		# atraverso una operazione di sottrazione con 65  
	  	...
	  	slt	$t1, $t0, $zero		# Se la sottrazione e' minore di zero (quindi non A B C D o E)
		beq	$t1, 1, goNext		# Allora vado all'elemento successivo
	  	bgt	$t0, 4, fineCore		# 
...
algorithmTable: la 	$t7, algorithmJAT	# Salvo l'indirizzo della JAT in $t7
		la	$t6, algorithm_A		# In $t6 metto l'indirizzo all'algoritmo A
		sw	$t6, 0($t7)		# E lo salvo nella JAT	
		la	$t6, algorithm_B		# In $t6 metto l'indirizzo all'algoritmo B
		sw	$t6, 4($t7)		# E lo salvo nella JAT
		la	$t6, algorithm_C		# In $t6 metto l'indirizzo all'algoritmo C
		sw	$t6, 8($t7)		# E lo salvo nella JAT
		la	$t6, algorithm_D		# In $t6 metto l'indirizzo all'algoritmo D
		sw	$t6, 12($t7)		# E lo salvo nella JAT
		la	$t6, algorithm_E		# In $t6 metto l'indirizzo all'algoritmo E
